...
class DQN:
    def __init__(self, name, env, seed=0,
                 ...
                 lr=1e-3, n_hidden_layers=2, n_neurons=64, notebook=True,
    ...
    def train(self):
        ...
        for epoch in range(self.current_epoch, self.n_epochs):
            ...
            for t in range(self.steps_per_epoch):
                ...
                # logging
                self.critic.eval()
                critic_per_ep.append(self.critic(obs)[act].detach().item())
                self.critic.train()
                obs = next_obs
            ...
            if self.current_epoch % 10 == 0:
                ...
                save_tb_scalars(self.writer,
                                self.current_epoch,
                                reward_per_episode=torch.tensor(reward_per_ep).sum() / self.steps_per_epoch,
    ...
    def get_action(self, obs, deterministic=False):
        ...
        if random.random() > self.eps or deterministic:
            ...
            return random.choice([a for a in range(self.env.action_space.n)])
